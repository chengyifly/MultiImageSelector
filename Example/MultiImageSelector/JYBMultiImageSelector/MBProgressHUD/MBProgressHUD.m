...
- (void)layoutSubviews {
	...
	CGSize labelSize = MB_MULTILINE_TEXTSIZE(label.text, label.font, CGSizeMake(maxWidth, bounds.size.height), label.lineBreakMode); // MARK::修改过，因为原tips只能显示一行 // [原] CGSize labelSize = MB_TEXTSIZE(label.text, label.font);
	labelSize.width = MIN(labelSize.width, maxWidth);
	totalSize.width = MAX(totalSize.width, labelSize.width);
	totalSize.height += labelSize.height;
	if (labelSize.height > 0.f && indicatorF.size.height > 0.f) {
		totalSize.height += kPadding*0.5; // MARK::修改过，因为带tips时，高度过高 // [原] totalSize.height += kPadding;
	}
